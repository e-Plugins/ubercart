...
class TargetPayCore
{
    ...
    public function parseCheckApi($strResult)
    {
        ...
        if(is_array($_result) && count($_result) > 1) {
            list($resultCode, $invoicePaymentReference, $status) = $_result;
        } else {
            $resultCode = $strResult;
        }
    
        if (trim($resultCode) == "000000 OK" ||
            (substr(trim($resultCode), 0, 6) == "000001" && $this->payMethod == 'CC' && $this->testMode == 1) ||    //For CC in test mode
            (substr(trim($resultCode), 0, 6) == "000000" && trim($status) == 'Captured')) { // Captured for Afterpay
